...
*2026-06-28 PART 4 (BUG-N12 — notify.STD_* groups + tablets.yaml broken since 2026-05-12 mobile_app migration):*
*User reported a recurring HA repair item: "Prepaid Strategic Top-Up Suggestion uses an unknown action: notify.std_warning." Root cause: configuration.yaml's four `notify: platform: group` definitions (STD_Information, STD_Warning, STD_Critical, STD_Alerts) referenced legacy mobile_app service names (`mobile_app_iphone16promax_ryan`, `mobile_app_ap_0223_1001`, `mobile_app_honor10_dash`, `mobile_app_honorx7_dash`) that stopped existing when the mobile_app integration migrated to per-device notify *entities* — entity registry shows the rename at 2026-05-12 (current names: `notify.ryan_iphone16_mobile_app`, `notify.ap_0223_1001`, `notify.honor_10_dash_mobile_app`, `notify.honor_x7_dash_mobile_app`). Since none of the old names resolved, all four `notify.STD_*` groups silently failed to set up at every HA start — not specific to the prepaid automation, every consumer of script.notify_power_event/notify_*_event was equally broken; the prepaid one just happened to be the automation that fired and surfaced the repair item first. Fixed: updated all four groups to the current entity-derived service names (every notify entity is also directly callable as `notify.<slug>`).*
*SAME BUG FOUND DIRECTLY IN CODE: `packages/admin/tablets.yaml` (tablet screen brightness automations) called `notify.mobile_app_honor10_dash` / `notify.mobile_app_honorx7_dash` at 8 call sites — same dead legacy names, same fix applied (→ `notify.honor_10_dash_mobile_app` / `notify.honor_x7_dash_mobile_app`). These had been silently failing since 2026-05-12 with no user-visible symptom until checked.*
*`ha core check` passed after both fixes. DOCS: NOTIFICATIONS_CONTRACT.md Section 7 (Platform Delivery Targets) updated with current entity names and the bug note.*

*2026-06-28 PART 3 (BUG-S52/BUG-P14/BUG-L10 — alert run-through requested by user: classifier mislabel, stuck staff override, evening arrival lighting):*
...
*DOCS: SECURITY_CONTRACT.md (BUG-S52, classifier output states table, debounce reduction note), PRESENCE_CONTRACT.md (BUG-P14, Section 11), LIGHTING_CONTRACT.md (BUG-L10) updated.*
