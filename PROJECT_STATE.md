...
packages/
  ...
  alerts/         # 13 files — see ALERTS_CONTRACT.md for actual file list (alerts_garden.yaml added 2026-04-29)
  ...
  presence/       # 5 files  — presence_helpers, presence_core, presence_confidence,
                 #             presence_boundary, presence_validation
  context/        # 4 files  — context_global, context_night, context_presence (trust model), context_schedules (stub)
                 #             VIOLATION: context_presence.yaml owns trust model — should move to presence/
...
|---|---|---|
| 🛡️ Security | Camera faults active | alert.camera_health implemented 2026-04-29: video loss on cam01/cam04/cam12 (NVR "NO VIDEO" — hardware/NVR channel issue, fix in Hikvision NVR UI). cam12 entity mapping confirmed: NVR Cam12-Back-Pond = cam11_back_pond entity IDs in HA (NVR channel renumbering post-config). binary_sensor.cam11_back_pond_videoloss currently ON. | Trust chain fixed. Inside cameras arming implemented. Triple-fire FIXED 2026-04-16: all three intruder automations now trigger on binary_sensor.security_intruder_active (from: off → on) with zone conditions. |
