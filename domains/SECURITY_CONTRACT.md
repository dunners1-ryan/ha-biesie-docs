...
### BUG-S69 — `security_event_classification`'s `reason` attribute used a derived/lagging `perim` aggregate, disagreeing with its own title
**Priority: HIGH | Status: ✅ FIXED 2026-07-17**

**Symptom:** Live push: title "⚠️ Perimeter activity" / "Activity outside boundary, no
presence explanation", but body read `zones: Beds | ... | cam: cam15_passage` — cam15 is
an inside passage camera, not a perimeter one, so the body flatly contradicted its own
title and made it look like an inside camera had been mislabelled as a perimeter event.

**Root cause:** The classifier's `state:` block (which produced the `perimeter_threat`
title) computes `perim` directly: `is_state(security_perimeter_front_motion,'on') or
is_state(security_perimeter_rear_motion,'on')`. The `reason` attribute — same sensor,
rendered in the same pass — instead read `binary_sensor.security_perimeter_motion`
(`security_zones.yaml`), a *derived* template sensor built from those exact same two
entities one hop removed. Derived/chained template sensors in this codebase lag a render
cycle behind their inputs (same class as BUG-NET05/06/S63) — so at the instant a real
perimeter camera fired, the fast path (`state:`) saw it immediately and correctly titled
the event "Perimeter activity", while the slow path (`reason`, via the aggregate) hadn't
caught up yet and only reflected the concurrently-active cam15 passage motion.

**Fix:** `reason`'s `perim` now computed identically to `state:`'s — directly from
`security_perimeter_front_motion`/`security_perimeter_rear_motion`, no aggregate hop.
`security_logic.yaml` modified.

---

### BUG-S70 — Notification "Camera:" field still wrong after BUG-S65 (regression risk introduced by that fix)
**Priority: MEDIUM | Status: ✅ FIXED 2026-07-17**

**Symptom:** Live push for a `grounds_low_confidence` (cam15_passage) event showed
"Camera: Cam12-Back-Pond" while the attached image was correctly Cam15-Passage.

**Root cause:** `cam_name`'s fallback tier reads `input_text.security_last_motion_camera`
— a GLOBAL "whoever fired most recently anywhere on the property" tracker (see BUG-S58).
BUG-S65 (earlier today) made this tracker update on *every* motion event instead of only
medium/high-confidence ones, to fix stale images — but for notifications using a per-ZONE
image file (the common case: gate/arrival/departure/visitor/grounds_low_confidence/
perimeter_threat), that same change made the "Camera:" field *more* likely to drift,
since now any camera firing anywhere on the property in the few seconds between
classification and delivery overwrites it, even though the image itself stays correctly
zone-locked. Confirmed live: cam12 fired a few seconds after the cam15 event classified,
flipping the global tracker before the notification actually sent.

**Fix:** `notify_security_events.yaml`'s `cam_name` now has a new top-priority tier —
`reason_cam`, extracted via regex from the message text's own `cam: <slug>` (present in
every branch whose message embeds `{{ reason }}`, i.e. exactly the branches that also use
a per-zone image). This text is generated by `security_event_classification`'s `reason`
attribute at classification time, from the same `trigger.to_state` the image's
`zone_label` was derived from — guaranteed consistent with the attached image, unlike the
volatile global tracker. Falls through to the pre-existing slug/entity/regex tiers
unchanged for branches that don't embed `reason` in their message (fully backward
compatible). `notify_security_events.yaml` modified.
