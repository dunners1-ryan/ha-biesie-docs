...
| Panels | 24× 430W JA Solar, installed 2021, north-facing (9.89 kWp nameplate, 23 usable × 430W). 1 unit damaged → 23 usable (the damaged unit itself is rated 435W, user-confirmed — not the fleet spec; a prior "24× 435W" label elsewhere was a mislabel, corrected 2026-08-21). Temporarily fully disconnected 2025-09-09→13 for roof maintenance (string reconnect confirmed via recorder data — see `private_docs/POWER_SYSTEM_AUDIT_2026.md` §3.2); still within panel warranty (2021 install) at time of writing |
...
System capex only (not personal budget/cash-flow — that stays in
`private_docs/SOLAR_UPGRADE_ROI_2026.md`, never here):
...
| Scheduled 2026-08-31 (was 2026-08-28, slipped — see below) | 12× 620W Canadian Solar bifacial panels + inverter swap (2×5.5kW SunSynk → 12kW Deye) via Vantage quote, not yet executed | ~R64,284 (R54,684 quote + R9,600 panels) | R10,000 net inverter trade-in already reflected in quote total | ~R64,284 |
...
9.89kWp array's range (see `POWER_SYSTEM_PERFORMANCE_LOG.md`'s 2026-08-29 entry), not the
17.33kWp post-upgrade profile. No reason given for the slip; not yet a concern.
...
`private_docs/SOLAR_UPGRADE_ROI_2026.md` (pending panel/inverter swap).
