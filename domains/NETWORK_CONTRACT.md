...
Connected" pushes in 40 minutes (14:02/14:07/14:12 SAST 2026-08-04) while the UniFi
console showed every device healthy with continuous uptime (AP Garage 15d+, never
rebooted). Confirmed via state-history API: `group.network_devices` (all 5 APs + ZenWiFi)
blipped `unavailable` for ~2 seconds each time then self-recovered — the signature of a
UniFi integration/websocket reconnect, not a real outage (same class of blip as BUG-NET06).
The domain's documented anti-flap gate worked correctly for all 3 blips —
`binary_sensor.network_device_down_alert_active` never reached `on` (its 250s `for:`
requirement was never met) and `alert.network_alert` stayed `idle` throughout. The false
pushes came from a *different* path: `automation.route_network_device_down_alert`
(~line 722) has a second trigger — `sensor.network_device_down_alert_severity`
transitioning `to: critical`, with **no `for:` duration** — that bypasses the 250s
anti-flap entirely. A full-group blip always computes `severity=critical` (6 devices down
