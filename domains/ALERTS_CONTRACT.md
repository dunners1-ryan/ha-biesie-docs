...
# 280→295.
#
# Previously — 2026-09-15 — BUG-A25: `binary_sensor.security_alert_active` no longer
...
| Stage | Entity | Status |
|---|---|---|
| Toggle | `input_boolean.security_alert_notify` | ✅ suppress pipeline escalations when cameras over-trigger. **2026-09-20 (SECURITY_CONTRACT.md BUG-S85):** now ALSO mutes every threat push from `security_event_router` (logged as `MUTED …`) and the pool siren — previously it only gated this pipeline, so muting during the Fri 18-Sep storm silenced nothing. Cleared daily by `automation.security_alert_mute_morning_reset` (06:00, `input_datetime.security_alerts_morning_reset`). |
| Binary sensor | `binary_sensor.security_alert_active` | ✅ delay_on 5s, checks security_system_enabled + alert_notify — no delay_off, so it directly tracks `sensor.security_threat_level`'s own stability; see SECURITY_CONTRACT.md BUG-S78 (2026-09-07) for an incident where an undebounced upstream camera signal made this cycle ~1000+ times in 16h. **2026-09-15 (BUG-A25):** `warning`/`critical` still trigger unconditionally, but `elevated` only counts while `binary_sensor.anyone_connected_home` is off — previously any `elevated` reading (including the documented daytime/family-home catch-all in `security_threat_level`) was push-worthy regardless of presence. Also: the "Camera:" field in `alert.security_alert` + `security_alert_repeat_reminder` now reads `sensor.security_trigger_camera` instead of the unrelated global `input_text.security_last_motion_camera`, so it's always drawn from the same live instant as the "Path:" field (`sensor.security_movement_path`). **2026-09-17 (BUG-A26):** the repeat-reminder call now also attaches `input_text.security_last_motion_image` as `image:` — previously the only `notify_security_event` caller in the repo with no image field at all. |
...
| Binary sensor | `binary_sensor.water_alert_active` | ✅ delay_on 30s; safety abort alone does NOT fire when water_state='ok'; **and (2026-09-25, BUG-A27/W02) not while `water_state='refilling'` with the pump on** — a lingering abort flag no longer counts as a bad state once a new refill is running. `route_water_tank_alert`'s `escalate` trigger (context→critical) now only passes when `water_alert_active` is already on, so an event is delivered once |
...
### BUG-A25 — `binary_sensor.security_alert_active` treated daytime `elevated` threat as push-worthy even while everyone's home — dog/leaf motion nagged every 5/15/30/60min

**Severity:** Medium
**File:** `packages/alerts/alerts_security.yaml`
**Status:** ✅ FIXED 2026-09-15

**Reported by:** user, from a live "⏰ Security Alert still active (5min)" push (`Threat:
ELEVATED Score: 95% Path: Rear Left`, `Camera: Cam15 Passage`) — "the security notice also
fired today which seems triggered by dogs or leaves moving which seems very sensitive and
also i am home so shouldn't be firing in day?"

**Root cause:** this repeat-reminder pipeline is entirely separate from the security
classifier/router (`security_event_router`, which already has RUNG 3 `family_movement` —
silent, logbook-only, for grounds/inside motion while anyone's home). `binary_sensor.
security_alert_active`'s own trigger condition treated `sensor.security_threat_level` in
...
equivalent to the classifier's RUNG 3 discount for it, so it pushed immediately and then
repeated via `security_alert_repeat_reminder` every 5/15/30/60min for as long as the
...
**Deployed:** YAML-only (1 file, 1 template sensor) — needs `ha core check` + a template
reload. Not yet live-verified against a real daytime dog/leaf event with someone home —
...
> **Superseded for the repeat reminder 2026-09-25 (SECURITY_CONTRACT BUG-S86):** `security_alert_repeat_reminder` no longer reads `sensor.security_trigger_camera` in its message text nor the global last-motion tracker/image. It resolves ONE camera (classifier `camera` attr → trigger sensor → tracker), passes it as `camera_override`, and snapshots that camera. The description below is historical for the reminder; the alert-context `devices` string and `alert.security_alert`'s (undelivered) message still use `sensor.security_trigger_camera`.
...
### BUG-A26 — `security_alert_repeat_reminder` never attached an image, unlike every other `notify_security_event` caller in the repo

**Severity:** Low
**File:** `packages/alerts/alerts_security.yaml`
**Status:** ✅ FIXED 2026-09-17

**Reported by:** user, from a screenshot showing 3 consecutive "⏰ Security Alert still
active" pushes with no photo (next to one real classifier-driven "INTRUDER" push that
did carry one) — "What is the point of these security threat alerts without images?"

**Root cause:** `security_alert_repeat_reminder`'s `script.notify_security_event` call
passed `severity`/`title`/`message`/`source`/`actions`/`telegram_action` — no `image:`
field at all, unlike every other caller of this script in the repo
(`security_automations.yaml`'s router branches, all of which attach a zone-matched
image). Not a severity- or time-based decision — the field was simply never added when
this automation was created (2026-07-10, closing a different PROJECT_STATE TODO about
`alert.security_alert`'s own broken `notifiers:`, BUG-A10). The reminder's actual job —
nagging that an unresolved threat is still active every 5/15/30/60min, since the alert
integration can't self-deliver — never needed an image to do that, so the gap went
unnoticed until a user asked why these specific pushes looked different from the rest.

**Fix:** added an `image:` field, sourced from `input_text.security_last_motion_image` —
the GLOBAL "last motion anywhere" tracker, unconditionally kept fresh on every motion
event since SECURITY_CONTRACT.md BUG-S65/S75. Not zone-matched the way the router's
per-branch images are (this reminder already reads other globals the same way, e.g.
`sensor.security_trigger_camera` per BUG-A25's follow-up above), but always recent.
Guarded (`candidate if candidate not in ['unknown','unavailable',''] else ''`) so a
genuinely-stale-tracker case degrades to no image via the script's own `{% if image %}`
skip, rather than a broken URL.

**Deployed:** YAML-only (1 automation, new `variables:` block + 1 field) — needs
`ha core check` + `Reload Automations`. Not yet live-verified — next repeat-reminder
firing should confirm an image renders.
...
| Security | ✅ | ✅ | ✅ | ✅ (triggered) | PASS | 2026-04-14 BUG-A02; repeat reminders base implementation shipped 2026-07-10 (see BUG-A10); 2026-08-18 Cancel Alert added, replacing the global-mute workaround (BUG-A19); 2026-09-15 `elevated` tier no longer push-worthy while anyone's home (BUG-A25); 2026-09-17 repeat reminder now attaches an image (BUG-A26) |
...
| BUG-A27 | **Medium** | ✅ Fixed 2026-09-25 | Water safety abort pushed 3× for one false event (24-Sep 10:36–10:40): `route_water_tank_alert`'s immediate `context→critical` trigger raced its `alert_active`-for-20s trigger (duplicate), and `refilling + pump on + abort flag still set` counted as a bad state (third push). Fixed in `alerts_water.yaml`; root cause (false no-rise trip) is WATER_CONTRACT Issue 21. |
| BUG-A26 | **Low** | ✅ Fixed 2026-09-17 (camera/image sourcing superseded 2026-09-25 — SECURITY_CONTRACT BUG-S86) | `security_alert_repeat_reminder`'s notify call had no `image:` field at all — the only `notify_security_event` caller in the repo without one. Now attaches `input_text.security_last_motion_image`, guarded against unknown/unavailable/empty | alerts_security.yaml |
...
**Open: 0 issues**  
**Fixed 2026-09-25: BUG-A27**  
**Fixed 2026-09-17: BUG-A26**
**Fixed 2026-09-15: BUG-A25**
...
*Last updated: 2026-09-25 — Rule 5b `initial:` audit: `initial:` removed from every `*_alert_snoozed` helper (device/dash/device-battery/device-power, gate, garden, media, device-restart, network-device-down, wan-down/degraded, power, presence, security, critical-sensor-health, 4 temperature, water/borehole ×2) and from the automation-muted `laundry_door_alert_notify` / `security_alert_notify`, so an active Cancel-Alert snooze or mute survives a Core restart instead of re-notifying. Plain `*_notify` on/off toggles keep `initial: true` (a restart reverts a manual mute to the safe default). `alerts_power.yaml` battery-low logic now reads `sensor.inverter_battery_soc` (POWER_CONTRACT Issue 37). `docs/Testing/Alert_Test_Plan.md` Power test section flagged for re-run.*
